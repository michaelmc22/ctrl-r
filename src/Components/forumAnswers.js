...
import "../Styles/forumasnwers.css";


function ForumAnswers() {
    const [forum, setForum] = useState([]);
    const forumCollectRef = collection(db, "gameforumans");
    ...
    const CreateForum = async (id) => {
        ...
        await addDoc(forumCollectRef, {
            ...
            alert("You must be logged in to create comment")
        }
    };
    ...
    const UpdateForum = async (id) => {
        ...
        const forumDoc = doc(db, "gameforumans", id)
        const newFields = { 
            answer: newQuestions, 
            date: date, 
            forumid: params.forumid, 
